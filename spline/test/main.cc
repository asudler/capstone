...
void makedata(int argc, char *argv[])
{
    ...
    double xmax = 0.; // max value for data
    ...
    if(ni == 0 || xmax == 0.) throw std::invalid_argument("makedata: "
...
void test_cubic_spline(int argc, char *argv[])
{
    ...
    if(fname == "") throw std::invalid_argument("test_cubic_spline: "
...
{   
    for(int i = 0; i < argc; ++i)
    {
        ...
        if(std::string(argv[i]) == "--test") test_cubic_spline(argc, argv);
    }
    return 0;
} // main
