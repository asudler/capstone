...
void matrix<T>::set(size_t i, size_t j, T x) { data[j + i*size2] = x; }

template <typename T> // getter
T matrix<T>::get(size_t i, size_t j) const { return data[j + i*size2]; } 

template <typename T> // operator ()
T& matrix<T>::operator()(size_t i, size_t j) { return data[j + i*size2]; }

template <typename T> // operator () again but read only (is it necessary?)
const T& matrix<T>::operator()(size_t i, size_t j) const 
{ 
    return data[j + i*size2]; 
}
...
matrix<T> matrix<T>::transpose()
{
    ...
    for(int i = 0; i < size1; i++)
    {
        ...
        {
            M.data[i + j*size1] = data[j + i*size2];
        }
    }
    return M;
}
...
std::vector<T> matrix<T>::row(int m)
{
    ...
    for(int i = 0; i < size2; i++) v[i] = data[m*size2 + i];
    return v;
}
...
std::vector<T> matrix<T>::column(int n)
{
    ...
    for(int i = 0; i < size1; i++) v[i] = data[n + i*size2];
    return v;
}
